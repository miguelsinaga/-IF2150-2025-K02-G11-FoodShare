...
class ProviderDashboard(ctk.CTkFrame):
    ...
    def create_stock_row(self, parent, item):
        ...
        # Actions dengan Hover dan Cursor
        btn_edit = ctk.CTkButton(row, text="Edit", width=60, fg_color="white", border_width=1, border_color="#DDD", 
                                 text_color="#333", hover_color="#F0F0F0", cursor="hand2")
        btn_edit.pack(side="left", padx=5)
        
        btn_del = ctk.CTkButton(row, text="Remove", width=70, fg_color="white", border_width=1, border_color="#EF4444", 
    ...
    def render_food_requests(self):
        ...
        for f in ["All", "Requested", "Preparing", "On Delivery", "Completed"]:
            is_active = (f == "All") # Logic sederhana, bisa dikembangkan
            fg = "#132A13" if is_active else "white"
            tc = "white" if is_active else "#333"
            hover = "#1F381F" if is_active else "#F0F0F0"
            
            ctk.CTkButton(filter_frame, text=f, fg_color=fg, text_color=tc, width=80, 
                          corner_radius=20, border_width=1, border_color="#DDD",
                          hover_color=hover, cursor="hand2").pack(side="left", padx=5)
    ...
    def create_request_row(self, parent, req):
        ...
        ctk.CTkLabel(row, text=f"Donasi #{req.idDonasi}", width=150, anchor="w", text_color="#333").pack(side="left", padx=10) 
        ctk.CTkLabel(row, text=f"User #{req.idReceiver}", width=120, anchor="w", text_color="#333").pack(side="left", padx=10)
        ...
        # Action Buttons dengan Hover
        if req.status == "Pending":
            ctk.CTkButton(row, text="Accept", width=60, fg_color="#10B981", hover_color="#059669", text_color="white", cursor="hand2").pack(side="left", padx=2)
            ctk.CTkButton(row, text="Reject", width=60, fg_color="white", border_width=1, border_color="red", 
                          text_color="red", hover_color="#FEF2F2", cursor="hand2").pack(side="left", padx=2)
        else:
            ctk.CTkButton(row, text="Update", width=80, fg_color="#132A13", hover_color="#1F381F", text_color="white", cursor="hand2").pack(side="left", padx=2)

        ctk.CTkFrame(parent, height=1, fg_color="#EEE").pack(fill="x")

    # ==============================================================
    # 4. FEEDBACK & 5. PROFILE (Sama seperti sebelumnya, tambah cursor)
    # ==============================================================
    ...
    def create_table_header(self, parent, headers):
        ...
        if len(headers) == 6: widths = [60, 150, 120, 100, 100, 100]
